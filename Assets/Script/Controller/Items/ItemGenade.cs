...
public class ItemGenade : ItemBomb
{
    ...
    protected virtual void Update()
    {
        ...
        if (timeShoot >= 0)
        {
            ...
            if (timeShoot < 0 && isUsed && PV.IsMine)
            {
                ...
                PV.RPC("BombBang", RpcTarget.All, GlobalParameter.playerName);
            }
        }
        if (isUsed && Input.GetMouseButtonUp(0))
        {
            if (delayShoot <= 0 && PV.IsMine)
            {
                ...
                PV.RPC("BombAddForce", RpcTarget.All, timeShoot, dir, GlobalParameter.playerName);
            }
        }
    ...
    [PunRPC]
    void BombBang(string playerName)
    {
        bombController = CreateController.Instance.CreateGenade(transform.position, ((BombInfo)itemInfo).damage, playerName);
        bombController.transform.SetParent(transform);
        bombController.Bang(GlobalParameter.playerName, ((BombInfo)itemInfo).damage);
    }

    [PunRPC]
    void BombAddForce(float time, Vector3 dir, string playerName)
    {
        isUsed = false;
        ...
        if (bombController != null)
        {
            ...
            if (amount <= 0 && controller.PV.IsMine) controller.GetComponent<PlayerEquipItem>().EquipGun(0);
        }
